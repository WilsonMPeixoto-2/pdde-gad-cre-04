...
export const PopSidebar = ({ activeSection, onSectionClick, isOpen, onClose }: PopSidebarProps) => {
  ...
  return (
    <>
      {isOpen ? (
        <div
          className="fixed inset-0 z-40 bg-slate-950/65 backdrop-blur-[2px] transition-opacity duration-200 lg:hidden"
          onClick={onClose}
          aria-hidden="true"
        />
      ) : null}
      ...
      <aside
        className={cn(
          "fixed left-0 top-0 z-50 h-screen w-[min(17.5rem,88vw)] transform border-r border-white/10 bg-slate-950 text-white transition-transform duration-200 no-print lg:sticky lg:top-[3.9rem] lg:z-30 lg:h-[calc(100vh-3.9rem)] lg:transform-none",
          isOpen
      ...
      >
        <div className="flex h-full flex-col">
          <div className="border-b border-white/10 px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[0.6rem] border border-white/12 bg-white/[0.055] text-sky-300" aria-hidden="true">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[0.68rem] text-white/65">4ª CRE · GAD</p>
                </div>
              </div>
              <Button
                variant="ghost"
                size="icon"
                onClick={onClose}
                className="h-8 w-8 text-white/72 hover:bg-white/[0.07] hover:text-white lg:hidden"
                ...
                return (
                  <li key={section.id}>
                    <button
                      ...
                      className={cn(
                        "relative flex w-full items-center gap-3 rounded-[0.65rem] border px-3 py-2.5 text-left transition-[background-color,border-color,color] duration-150 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-sky-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950",
                        isActive
                    ...
                    >
                      <span
                        className={cn(
                          "flex h-7 w-7 shrink-0 items-center justify-center rounded-[0.45rem] border text-[0.72rem] font-bold tabular-nums",
                          isActive
                      ...
                      <span className="min-w-0 flex-1 truncate text-[0.82rem] font-semibold leading-5">
                        {section.shortTitle}
                      </span>
                      ...
                      <span className="absolute inset-x-3 bottom-0 h-px overflow-hidden bg-white/5" aria-hidden="true">
                        <span
                          className={cn("block h-full", isRead ? "bg-emerald-400/65" : "bg-sky-400/55")}
          ...
          <div className="border-t border-white/10 px-4 py-3.5 text-[0.68rem] leading-5 text-white/58">
